...
export default function Page() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiMessages = Array.isArray(data.messages)
        ? data.messages
        : [{ role: 'assistant', content: data.content || 'Sorry, I received an unexpected response.' }];
  ...
  const handleSaveEdit = async (index: number) => {
    ...
    // Keep the same type as original content
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 min-h-0 scrollbar scrollbar-thumb-[#5c2d2d] scrollbar-track-[#2a1414] scrollbar-w-2">
        ...
        {messages.map((message, index) => {
          ...
          >
                  ...
                  <div className="prose prose-sm prose-invert text-[#e8d5b5] mb-2 relative">
                    <ReactMarkdown
                      components={{
                        em: ({ node, ...props }) => <em className="text-[#757575]" {...props} />,
                        strong: ({ node, ...props }) => <strong className="text-[#c49c6b]" {...props} />,
                ...
                {reasoning && (
                  <details className="mb-3">
                    ...
                    <div className="mt-4 p-2 bg-[#3a2020] rounded prose prose-sm prose-invert text-[#d4b58c]">
                      <ReactMarkdown
                        components={{
                          em: ({ node, ...props }) => <em className="text-[#757575]" {...props} />,
                          strong: ({ node, ...props }) => <strong className="text-[#c49c6b]" {...props} />,
                ...
                <div className="mt-3 border-t border-[#5c2d2d]/50"></div>
              </div>
            );
          })}
